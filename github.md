# Bienvenida a GH-900

## Introducción
*GitHub* es una plataforma líder para la colaboración en desarrollo de software, que permite a equipos y comunidades trabajar juntos de manera eficiente.

**Material del curso:**
- Fundamentos de Git y GitHub
- Colaboración y administración de proyectos
- Seguridad y buenas prácticas
- Uso de herramientas modernas como GitHub Copilot y Codespaces

<p align="center">
  <img src="https://logos-world.net/wp-content/uploads/2020/11/GitHub-Logo.png" width="100" />
</p>

---

## Contenido

### Día 1
- Introducción a Git  
- Introducción a GitHub  
- Introducción a los productos de GitHub  
- Configuración del examen de código en GitHub  
- Introducción a GitHub Copilot  
- Código con GitHub Codespaces  
- Administración del trabajo con GitHub Projects  
- Comuníquese eficazmente en GitHub con Markdown  

### Día 2
- Contribución a un proyecto de código abierto en GitHub  
- Administración de un programa InnerSource mediante GitHub  
- Mantenimiento de un repositorio seguro mediante procedimientos recomendados de GitHub  
- Introducción a la administración de GitHub  
- Autenticación y autorización de identidades de usuario en GitHub  
- Administración de cambios de repositorio por medio de solicitudes de incorporación de cambios en GitHub  
- Organización del historial de un repositorio y búsqueda mediante GitHub  
- Uso de GitHub Copilot con Python  

---


## Conclusión
Este curso te proporcionará las habilidades esenciales para trabajar con GitHub de manera profesional y segura.  
[GH-900 - GitHub Foundations](https://learn.microsoft.com/en-us/certifications/exams/gh-900)

### Evaluación del examen

| Dominio | Porcentaje |
|---------|-----------|
| Domain 1: Introduction to Git and GitHub | 22% |
| Domain 2: Working with GitHub Repositories | 8% |
| Domain 3: Collaboration Features | 30% |
| Domain 4: Modern Development | 13% |
| Domain 5: Project Management | 7% |
| Domain 6: Privacy, Security, and Administration | 10% |
| Domain 7: Benefits of the GitHub Community | 10% |

---
---

## Utilidad Python para unir archivos de Excel

Se agregó el script `unir_excels.py` para consolidar archivos `.xlsx`, `.xlsm`, `.xltx` o `.xltm` ubicados en la misma carpeta del script.

Uso básico:

```bash
pip install -r requirements.txt
python unir_excels.py
```

El script:
- valida que existan 29 archivos de Excel por defecto (o permite omitir esa validación con `--cantidad-esperada 0`)
- toma la primera hoja de cada archivo
- une todas las filas en una sola hoja
- ordena el resultado por la columna de fecha
- genera el archivo `excel_unido.xlsx`
